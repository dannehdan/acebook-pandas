describe('Creating comments', function () {
  ...
  it('have comment toggle button under post', () => {
    ...
    cy.get('.post').find('.comment-toggle-button').should('contain', 'Add comment');
  });
  ...
  it('toggler on click shows input field for comment and comment button', () => {
    ...
    cy.get('.comment-box-div').find('.comment-text').should('have.attr', 'required');
    cy.get('.comment-box-div').find('.comment-button').should('contain', 'Comment');
  });
  ...
  it('user can comment a post', () => {
    ...
    cy.get('.comments-div').find('.comment-message').should('contain', 'good point!');
  });
  ...
  it('can see only two most recent comments by default', () => {
    ...
    cy.get('.comments-div').find('.collapse-comment').should('contain', 'one').should('not.be.visible');
  });
  ...
  it('can see all comments after clicking "show more" button', () => {
    ...
    for (const comment of comments) {
      cy.get('.comments-div').find('.comment-message').contains(comment).should('be.visible');
    }
  });
  ...
  it('can see a name of the comment\'s author', () => {
    ...
    cy.get('.comments-div').find('.commenter-name').should('contain', 'Luna Lovegood');
  })
});
